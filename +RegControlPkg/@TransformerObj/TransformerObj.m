    ...
    methods
        function obj = SetNumWindings(obj, N)
            ...
            if ~isempty(obj.kVs)
                ...
                if len < MaxWindings
                    % leftover after resize:
                    for ii = (len + 1):MaxWindings
                        obj.Winding(ii).kvll = 0;
                    end
                end
            end
...
%                 Y_Term_NL  = TCMatrix.CreateMatrix(2 * NumWindings);
        ...
        function obj = TransformerObj(varargin)
            ...
            boolchk = {'logical'};
            strchk = {'string'};
            cellchk = {'cell', 'numeric'};
            
            nempty = {'nonempty'};
            posint = {'nonempty','integer','positive'};
            pospct = {'nonempty','positive'};
            
            addOptional(p,'kVs',double.empty,@(x)validateattributes(x,cellchk,nempty));
            addOptional(p,'kVAs',double.empty,@(x)validateattributes(x,cellchk,nempty));
...
%                 string(num2str(obj.kVs)), obj.pctLoadLoss);
            ...
            for ii = 1:obj.NumWindings
                ...
                line2 = line2 + compose( formatSpec, ...
                    ...
                    obj.Winding(ii).Rneut, obj.Winding(ii).Xneut, ...
            ...
            line3 = compose(formatSpec, string(obj.IsSubstation), ...
                string(obj.XRConst), (obj.pctImag / 0.01), ...
                (obj.pctLoadLoss  / 0.01), (obj.pctNoLoadLoss / 0.01));
            
            formatSpec = "emerghkVA=%g faultrate=%g flrise=%g hsrise=%g normhkVA=%g ";
            line4 = compose(formatSpec, obj.EmergMaxHkVA, (obj.FaultRate / 0.01), obj.FLrise, ...
                obj.HSrise, obj.NormMaxHkVA);
            
            formatSpec = "Xhl=%g Xht=%g Xlt=%g thermal=%g n=%g m=%g ppm_antifloat=%f ";
            line5 = compose(formatSpec, (obj.XHL / 0.01), (obj.XHT / 0.01), (obj.XLT / 0.01), obj.ThermalTimeconst, ...
    ...
    methods(Static)
        function obj = MakeLike(AnotherTransformer)
            ...
            args{6}  = p.Results.AnotherTransformer.NumWindings;  % Force allocation of terminals and conductors
